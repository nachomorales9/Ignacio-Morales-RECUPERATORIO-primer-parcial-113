...
def cargar_archivo_csv(archivo):
    ...
    with open(path, 'r', encoding='utf-8') as archivo_csv:
        ...
        for linea in lector:
            ...
            linea['user'] = linea['user']
            linea['likes'] = int(linea['likes'])
            linea['dislikes'] = int(linea['dislikes'])
            linea['followers'] = int(linea['followers'])
            posts.append(linea)
...
def filtrar_por_mejores_posts(posts):
    ...
    nombre_archivo = "mejores_posts.csv"

    with open(nombre_archivo, 'w', encoding='utf-8', newline='') as archivo_csv:
        parametros = ['id', 'user', 'likes', 'dislikes', 'followers']
        escritor = csv.DictWriter(archivo_csv, fieldnames=parametros)
        escritor.writeheader()
        escritor.writerows(mejores_posts)
...
def filtrar_por_haters(posts):
    ...
    with open(nombre_archivo, 'w', encoding='utf-8', newline='') as archivo_csv:
        ...
        escritor = csv.DictWriter(archivo_csv, fieldnames=parametros)
        escritor.writeheader()
        escritor.writerows(haters_posts)
